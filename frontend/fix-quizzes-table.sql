-- Create quizzes table and RLS policies
...
-- Add foreign key constraint only if articles table exists and has proper structure
DO $$
BEGIN
    -- Check if articles table exists and has id column
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            WHERE constraint_name = 'quizzes_article_id_fkey'
        ) THEN
            ALTER TABLE quizzes ADD CONSTRAINT quizzes_article_id_fkey 
            FOREIGN KEY (article_id) REFERENCES articles(id) ON DELETE CASCADE;
            RAISE NOTICE 'Added foreign key constraint to quizzes table';
        ELSE
            RAISE NOTICE 'Foreign key constraint already exists';
        END IF;
    ELSE
        RAISE NOTICE 'Articles table not found or missing id column, skipping foreign key constraint';
    END IF;
END $$;
...
BEGIN
    ...
    -- Authors can manage quizzes for their own articles (if articles table exists)
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'articles'
    ) AND EXISTS (
        ...
        CREATE POLICY "Authors can manage their article quizzes" ON quizzes
            FOR ALL USING (
                EXISTS (
                    SELECT 1 FROM articles 
                    WHERE articles.id = quizzes.article_id 
                    AND articles.author_id = auth.uid()
                )
            );
        RAISE NOTICE 'Created author policy for quizzes';
    ELSE
        -- Fallback: allow users to manage quizzes they created (if we track creator)
        CREATE POLICY "Users can manage their own quizzes" ON quizzes
            FOR ALL USING (auth.uid()::text = article_id::text); -- Simple fallback
        RAISE NOTICE 'Created fallback user policy for quizzes';
    END IF;
    
    -- Editors can manage all quizzes
    CREATE POLICY "Editors can manage all quizzes" ON quizzes
        FOR ALL USING (
            EXISTS (
...
BEGIN
    ...
    -- Editors can view all attempts (for analytics)
    CREATE POLICY "Editors can view all attempts" ON quiz_attempts
        FOR SELECT USING (
            EXISTS (
                SELECT 1 FROM profiles 
                WHERE id = auth.uid() 
